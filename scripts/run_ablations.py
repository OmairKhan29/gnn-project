...
def run_ablation(ablation_name, datasets, seeds, device, resume=False):
    ...
    if resume and os.path.exists(output_path):
        ...
        print(f"Resuming ablation '{ablation_name}' — {len(all_results)} value(s) already done.")
    ...
    for value in config["values"]:
        if resume and str(value) in all_results:
            print(f"\nSkipping {config['variable']} = {value} (already done)")
            continue
        print(f"\n{'─' * 60}")
        ...
        # Save progress after every value
    ...
    print(f"\nAblation '{ablation_name}' complete. Results: {output_path}")
    return all_results
...
def main():
    ...
    for ablation_name in ablations:
        run_ablation(ablation_name, args.datasets, args.seeds, args.device, args.resume)
